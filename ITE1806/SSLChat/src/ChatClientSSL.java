...
public class ChatClientSSL extends Application {
  ...
  /**
   * {@link SSLSocket} used to communicate back and forth with the server
   */
  private SSLSocket sslSocket;

  /**
   * {@link String} used to define the path to the truststore
   */
  private static final String tsPath = "rsa/truststore.store";

  /**
   * {@link char[]} used to store the password fot the truststore
   */
  private static final char[] tsPass = "#$%nOonEcaNcracKthiSonEsupeRsecreTpassworDeitheRyO!@&$".toCharArray();
  ...
  try {
    // creates a keystore to store our truststore
    KeyStore trustStore = KeyStore.getInstance("JKS");
    trustStore.load(new FileInputStream(tsPath), tsPass);

    // Creates and initializes a TrustManagerFactory
    ...
    // Creates the SSLSocketFactory and retrieves our SSLSocket
    SSLSocketFactory sslSocketFactory = sslContext.getSocketFactory();
    sslSocket = (SSLSocket) sslSocketFactory.createSocket("localhost", 1313);

    // Opens the socket
    sslSocket.startHandshake();

    // Sets up input and output streams with server
    fromServer = new ObjectInputStream(sslSocket.getInputStream());
