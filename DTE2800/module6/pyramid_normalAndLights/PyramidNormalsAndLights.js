...
// Lights
let u_lightDirection, u_ambientLightColor, u_diffuseLightColor;
...
function init() {
	...
	if (!(gl.pyramidShaderProgram = createProgram(gl, Light.getDirectionalVShaderSource("texture"), Light.getFShaderSource("texture")))) {
	...
	// Lights
	u_lightDirection = gl.getUniformLocation(gl.pyramidShaderProgram, "u_lightDirection");
...
function drawPyramid() {
	...
	lights.ambientLightColor = [0.5, 0.5, 0.5];
	lights.diffuseLightColor = [0.5, 0.5, 0.5];
	gl.uniform3fv(u_lightDirection, lights.lightDirection);
