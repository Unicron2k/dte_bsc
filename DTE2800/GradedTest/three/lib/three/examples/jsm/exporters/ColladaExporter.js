...
ColladaExporter.prototype = {
	...
	parse: function ( object, onDone, options ) {
		...
		// Process the given textures into the image library
			...
			if ( matid == null ) {
				...
				if ( m instanceof MeshLambertMaterial ) {
					...
					if ( m.map !== null ) {
						...
						// The Collada spec does not support diffuse textures maps with the
				...
				var techniqueNode = `<technique sid="common"><${ type }>` +
					...
					(
						m.emissiveMap ?
							'<textures textures="emissive-sampler" texcoord="TEXCOORD" />' :
					...
					(
						...
						(
							m.map ?
								'<textures textures="diffuse-sampler" texcoord="TEXCOORD" />' :
					...
					(
						...
						(
							m.specularMap ?
								'<textures textures="specular-sampler" texcoord="TEXCOORD" />' :
