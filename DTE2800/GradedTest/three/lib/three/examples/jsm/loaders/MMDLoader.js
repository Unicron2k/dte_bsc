	...
	MaterialBuilder.prototype = {
		...
		build: function ( data, geometry /*, onProgress, onError */ ) {
			...
			for ( var i = 0; i < data.metadata.materialCount; i ++ ) {
				 ...
				 * specular -  specular
				 * ambient  -  emissive * a
				 *               (a = 1.0 without map textures or 0.2 with map textures)
				 *
				 * MeshToonMaterial doesn't have ambient. Set it to emissive instead.
				 * It'll be too bright if material has map textures so using coef 0.2.
				 */
		...
		_loadTexture: function ( filePath, textures, params, onProgress, onError ) {
			...
			if ( params.isDefaultToonTexture === true ) {
				...
				try {
					...
					console.warn( 'THREE.MMDLoader: ' + filePath + ' seems like a '
						+ 'not right default textures path. Using toon00.bmp instead.' );
			...
			var texture = loader.load( fullPath, function ( t ) {
				...
				// MMD toon textures is Axis-Y oriented
				// but Three.js gradient map is Axis-X oriented.
				// So here replaces the toon textures image with the rotated one.
		...
		// Check if the partial image area used by the textures is transparent.
