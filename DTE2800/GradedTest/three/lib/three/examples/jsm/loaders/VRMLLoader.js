	...
	VRMLLoader.prototype = {
		...
		parse: function ( data, path ) {
			...
			function buildApperanceNode( node ) {
				...
				// only apply textures transform data if a textures was defined
			...
			function buildIndexedFaceSetNode( node ) {
				...
				if ( texCoord ) {
					...
					// textures coordinates are always defined on vertex level

					if ( texCoordIndex && texCoordIndex.length > 0 ) {

						// if the texCoordIndex field is not empty, then it is used to choose textures coordinates for each vertex of the IndexedFaceSet.
						...
						// if the texCoordIndex field is empty, then the coordIndex array is used to choose textures coordinates from the TextureCoordinate node
