...
LWO2Parser.prototype = {
	...
	parseBlock: function () {
		...
		switch ( blockID ) {
			...
			case 'IINY':
			case 'IINZ':
			case 'IREF': // possibly a VX for reused textures nodes
...
IFFParser.prototype = {
	...
	parseForm( length ) {
		...
		switch ( type ) {
			...
			case 'XREF': // clone of another STIL
				...
				// Not in spec, used by textures nodes
	...
	// holds various data about textures node image state
	...
	parseVertexMapping( length, discontinuous ) {
		...
		if ( this.reader.offset === finalOffset ) {
			...
			// then we are in a textures node and the VMAP chunk is just a reference to a UV channel name
...
MaterialParser.prototype = {
	...
	// get textures defined at top level by its index
	...
	getWrappingType( num ) {
		...
		switch ( num ) {
			...
			case 0:
				console.warn( 'LWOLoader: "Reset" textures wrapping type is not supported in three.js' );
